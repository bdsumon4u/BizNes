@props(['label' => null, 'placeholder' => 'Select Date', 'model' => 'date'])

<div class="flex items-center mt-1 gap-x-2">
    @isset($label)
    <label class="text-sm font-medium text-gray-700 dark:text-gray-200 whitespace-nowrap">{{ $label }}</label>
    @endisset

    <div x-modelable="datePickerValue" x-model="{{ $model }}" x-data="{
    ...
    datePickerCalculateDays();"
        ...
        <div class="relative w-40">
            <input x-ref="datePickerInput" type="text" @click="datePickerOpen=true" x-model="datePickerValue"
                x-on:keydown.escape="datePickerOpen=false"
                class="flex w-full px-3 py-1 text-sm bg-white border rounded-md text-neutral-600 border-neutral-300 ring-offset-background placeholder:text-neutral-400 focus:border-neutral-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-neutral-400 disabled:cursor-not-allowed disabled:opacity-50"
                placeholder="{{ $placeholder }}" readonly />
            <div @click="datePickerOpen=!datePickerOpen; if(datePickerOpen){ $refs.datePickerInput.focus() }"
                class="absolute top-0 right-0 z-10 px-2 py-1 cursor-pointer text-neutral-400 hover:text-neutral-500">
            ...
            <div x-show="datePickerOpen" x-transition @click.away="datePickerOpen = false"
                ...
                <div class="relative z-50 p-4 bg-white border rounded-lg shadow w-[17rem] border-neutral-200/70">
                        ...
                        <template x-for="(day, dayIndex) in datePickerDaysInMonth" :key="dayIndex">
                            <div class="px-0.5 mb-1 aspect-square">
                                <div x-text="day" @click="datePickerDayClicked(day)"
                                    :class="{
                                        'bg-neutral-200': datePickerIsToday(
                                            day) == true,
